...
export default function Hero({ onRegisterClick, lang, onPageChange }: HeroProps) {
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen flex flex-col items-center justify-center pt-28 pb-16 overflow-hidden bg-[#020617] bg-grid-white"
    >
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 relative z-10 w-full flex flex-col items-center">
        ...
        <motion.div
          ...
          className="w-full max-w-4xl bg-gradient-to-r from-blue-950/80 via-slate-900/90 to-amber-950/80 border border-blue-500/30 rounded-2xl p-3.5 mb-10 text-center flex flex-col sm:flex-row items-center justify-between gap-3 shadow-xl backdrop-blur-md"
        >
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-300 font-medium">
              ...
              <span className="font-bold text-white uppercase tracking-wider">{t.compWidgetBadge}</span>
              <span className="hidden md:inline text-slate-400">•</span>
              <span className="hidden md:inline text-amber-400">{t.compWidgetPrize}</span>
            </div>
          </div>
          <button
            onClick={() => {
            ...
            className="text-xs font-bold text-amber-400 hover:text-amber-300 transition-colors flex items-center gap-1 group/alert cursor-pointer shrink-0 bg-transparent border-none outline-none"
          >
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-400 text-xs font-semibold mb-6 hover:bg-blue-500/15 transition-colors cursor-default"
        >
        ...
        <motion.h1
          ...
          className="font-display text-center text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-extrabold tracking-tighter mb-6 leading-[0.95] text-white"
        >
          {t.brandName} <br />
        ...
        <motion.p
          ...
          className="text-center text-lg md:text-xl text-slate-400 max-w-3xl mb-8 leading-relaxed"
        >
          {t.heroSubtitle}
        </motion.p>
        ...
        <motion.div
          ...
          className="w-full max-w-4xl bg-slate-900/35 backdrop-blur-lg border border-slate-800/90 rounded-3xl p-6 md:p-8 mb-12 shadow-2xl relative overflow-hidden"
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-6 text-left space-y-4">
              <span className="text-[10px] font-mono tracking-widest text-amber-400 uppercase font-semibold bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/20">
                🏆 {t.compWidgetTitle}
              </span>
              <h3 className="text-xl md:text-2xl font-extrabold text-white tracking-tight leading-snug">
                {t.compWidgetHeading} <span className="text-amber-400">2026</span>
              </h3>
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                <div className="flex items-center gap-2 text-xs font-semibold text-emerald-400 bg-emerald-500/5 border border-emerald-500/10 px-3 py-1.5 rounded-xl">
                  <Trophy className="w-4 h-4" />
                  <span>{t.compWidgetPrize}</span>
                </div>
                <div className="flex items-center gap-2 text-xs font-semibold text-blue-400 bg-blue-500/5 border border-blue-500/10 px-3 py-1.5 rounded-xl">
            ...
            <div className="lg:col-span-6 bg-slate-950/80 border border-slate-800/80 rounded-2xl p-5 md:p-6 text-center space-y-4">
              <div className="flex items-center gap-2 justify-center text-[11px] font-mono tracking-widest text-slate-400 uppercase font-bold">
                <Calendar className="w-4 h-4 text-amber-500 animate-pulse" />
                <span>{t.compWidgetCountdown}</span>
              </div>

              <div className="grid grid-cols-4 gap-2.5 text-center">
                <div className="bg-slate-900 border border-slate-800/60 p-2.5 rounded-xl">
                  <div className="text-2xl md:text-3xl font-extrabold font-display text-white">
                    {String(timeLeft.days).padStart(2, '0')}
                  </div>
                  <div className="text-[9px] text-slate-500 uppercase tracking-wider font-mono mt-0.5">{t.days}</div>
                </div>
                <div className="bg-slate-900 border border-slate-800/60 p-2.5 rounded-xl">
                  <div className="text-2xl md:text-3xl font-extrabold font-display text-white">
                    {String(timeLeft.hours).padStart(2, '0')}
                  </div>
                  <div className="text-[9px] text-slate-500 uppercase tracking-wider font-mono mt-0.5">{t.hours}</div>
                </div>
                <div className="bg-slate-900 border border-slate-800/60 p-2.5 rounded-xl">
                  <div className="text-2xl md:text-3xl font-extrabold font-display text-white">
                    {String(timeLeft.minutes).padStart(2, '0')}
                  </div>
                  <div className="text-[9px] text-slate-500 uppercase tracking-wider font-mono mt-0.5">{t.minutes}</div>
                </div>
                <div className="bg-slate-900 border border-slate-800/60 p-2.5 rounded-xl text-amber-400">
                  <div className="text-2xl md:text-3xl font-extrabold font-display">
                    {String(timeLeft.seconds).padStart(2, '0')}
                  </div>
                  <div className="text-[9px] text-amber-500 uppercase tracking-wider font-mono mt-0.5">{t.seconds}</div>
                </div>
              </div>
        ...
        >
          ...
          >
            ...
            <Play className={`w-4 h-4 text-blue-400 fill-blue-400 ${lang === 'ar' ? 'rotate-180' : ''}`} />
          </a>
