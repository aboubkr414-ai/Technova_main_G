    ...
    <section id="team" className="py-24 bg-[#020617] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 relative z-10" id="team-container">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20" id="team-header">
          ...
          <h2 className="font-display text-4xl sm:text-5xl font-extrabold tracking-tight mb-4 text-white">
        ...
        <div className="space-y-16" id="team-categories">
          {teamCategories.map((category, catIdx) => (
            <div key={category.id} className="space-y-6" id={`cat-${category.id}`}>
              ...
              <div className="flex items-center gap-3 pb-3 border-b border-slate-800" id={`cat-title-${category.id}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id={`members-grid-${category.id}`}>
                {category.members.map((member, memberIdx) => (
                  <motion.div
                    ...
                    className={`group relative bg-slate-900/40 hover:bg-slate-900/60 border border-slate-800 hover:border-slate-700/80 rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row gap-6 transition-all duration-300 hover:shadow-xl`}
                    id={`member-card-${member.nameEn.toLowerCase().replace(/\s+/g, '-')}`}
                  >
                    {/* Member Avatar Circle */}
                    <div className="flex-shrink-0">
                      <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${member.color} border flex items-center justify-center font-display text-xl font-bold text-white shadow-inner group-hover:scale-105 transition-transform duration-300`}>
                        {member.initials}
                      </div>
                    </div>
                    ...
                    <div className="flex-grow space-y-2">
                      <div className="flex items-start justify-between">
                        <div>
                          <h4 className="text-lg font-bold text-white group-hover:text-amber-400 transition-colors duration-200">
                            {lang === 'ar' ? member.nameAr : member.nameEn}
                          </h4>
                          <p className="text-sm font-mono font-medium text-blue-400 tracking-tight">
        ...
        <motion.div
          ...
          className="mt-20 p-8 rounded-3xl bg-slate-900/30 border border-slate-800/80 backdrop-blur-md relative overflow-hidden"
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            ...
            <a
              ...
              className="inline-flex items-center justify-center gap-3 px-6 py-3 bg-slate-950 hover:bg-slate-900 border border-slate-800 hover:border-slate-700 text-slate-200 hover:text-white font-mono text-sm rounded-xl transition-all duration-300 group shadow-md"
              id="team-email-link"
            >
